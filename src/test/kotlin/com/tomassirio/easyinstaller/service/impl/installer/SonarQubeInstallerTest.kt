...
class SonarQubeInstallerTest {
    ...
    @InjectMocks
    private lateinit var sonarQubeInstaller: SonarQubeInstaller
    ...
    fun setUp() {
        ...
        val defaultCommand = properties.getProperty("url.default.sonarqube")

        ReflectionTestUtils.setField(sonarQubeInstaller, defaultCommandField, defaultCommand)
    }
    ...
    fun `test successful installation with provided package manager`() {
        ...
        verify(shellFormatter).printInfo("Installing SonarQube...")
        verify(strategy).install("sonarqube")
    }
    ...
    fun `test default curl URL is valid`() {
        ...
        val modifiedCommand = "curl --head ${sonarQubeInstaller.DEFAULT_URL}"
