...
class NixInstallerTest {
    ...
    @InjectMocks
    private lateinit var nixInstaller: NixInstaller
    ...
    fun setUp() {
        ...
        val defaultCommand = properties.getProperty("url.default.nix")

        ReflectionTestUtils.setField(nixInstaller, defaultCommandField, defaultCommand)
    }
    ...
    fun `test successful installation with default command`() {
        ...
        nixInstaller.install()

        verify(shellFormatter).printInfo("Installing Nix...")
        verify(strategy).invoke("curl -fsSL ${nixInstaller.DEFAULT_URL} | sudo bash")
    }
    ...
    fun `test default curl URL is valid`() {
        ...
        val modifiedCommand = "curl --head ${nixInstaller.DEFAULT_URL}"
        ...
        assertTrue(output.contains("HTTP/2 200")
                .or(output.contains("HTTP/2 302")), "Expected output to contain 'HTTP/2 200'. Output was: $output")
    }
}