...
class AptStrategyTest {
    ...
    @InjectMocks
    private lateinit var aptStrategy: AptStrategy
    ...
    fun `install should call process with correct command`() {
        ...
        aptStrategy.install(urlOrName)
        verify(shellFormatter).printInfo(commandCaptor.capture())
        assertEquals("With commands: apt install -y $urlOrName...", commandCaptor.value)
    }

    @Test
    fun `name should return correct name`() {
        assertEquals("apt", aptStrategy.name())
    }
}