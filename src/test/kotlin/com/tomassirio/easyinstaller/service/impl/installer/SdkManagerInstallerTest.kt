...
class SdkManagerInstallerTest {
    ...
    @InjectMocks
    private lateinit var sdkManagerInstaller: SdkManagerInstaller
    ...
    fun setUp() {
        ...
        val defaultCommand = properties.getProperty("url.default.sdkman")

        ReflectionTestUtils.setField(sdkManagerInstaller, defaultCommandField, defaultCommand)
    }
    ...
    fun `test successful installation with default command`() {
        ...
        sdkManagerInstaller.install()

        verify(shellFormatter).printInfo("Installing SdkManager...")
        verify(strategy).invoke("curl -fsSL ${sdkManagerInstaller.DEFAULT_URL} | sudo bash")
    }
    ...
    fun `test successful installation with provided package manager`() {
        ...
        verify(shellFormatter).printInfo("Installing SdkManager...")
        verify(strategy).install("sdkman")
    }
    ...
    fun `test default curl URL is valid`() {
        ...
        val modifiedCommand = "curl --head ${sdkManagerInstaller.DEFAULT_URL}"
