...
class HttpieInstallerTest {
    ...
    @InjectMocks
    private lateinit var httpieInstaller: HttpieInstaller
    ...
    fun setUp() {
        ...
        val defaultCommand = properties.getProperty("url.default.httpie")

        ReflectionTestUtils.setField(httpieInstaller, defaultCommandField, defaultCommand)
    }
    ...
    fun `test successful installation with provided package manager`() {
        ...
        verify(shellFormatter).printInfo("Installing Httpie...")
        verify(strategy).install("httpie")
    }
    ...
    fun `test default curl URL is valid`() {
        ...
        val modifiedCommand = "curl --head ${httpieInstaller.DEFAULT_URL}"
