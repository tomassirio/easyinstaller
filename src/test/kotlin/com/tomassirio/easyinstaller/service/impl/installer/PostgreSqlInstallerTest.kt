...
class PostgreSqlInstallerTest {
    ...
    @InjectMocks
    private lateinit var postgreSqlInstaller: PostgreSqlInstaller
    ...
    fun setUp() {
        ...
        val defaultCommand = properties.getProperty("url.default.postgresql")

        ReflectionTestUtils.setField(postgreSqlInstaller, defaultCommandField, defaultCommand)
    }
    ...
    fun `test successful installation with provided package manager`() {
        ...
        verify(shellFormatter).printInfo("Installing PostgreSql...")
        verify(strategy).install("postgresql")
    }
    ...
    fun `test default curl URL is valid`() {
        ...
        val modifiedCommand = "curl --head ${postgreSqlInstaller.DEFAULT_URL}"
