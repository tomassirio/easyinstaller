...
class EclipseInstallerTest {
    ...
    @InjectMocks
    private lateinit var eclipseInstaller: EclipseInstaller
    ...
    fun setUp() {
        ...
        val defaultCommand = properties.getProperty("url.default.eclipse")

        ReflectionTestUtils.setField(eclipseInstaller, defaultCommandField, defaultCommand)
    }
    ...
    fun `test successful installation with provided package manager`() {
        ...
        verify(shellFormatter).printInfo("Installing Eclipse...")
        verify(strategy).install("eclipse")
    }
    ...
    fun `test default curl URL is valid`() {
        ...
        val modifiedCommand = "curl --head ${eclipseInstaller.DEFAULT_URL}"
