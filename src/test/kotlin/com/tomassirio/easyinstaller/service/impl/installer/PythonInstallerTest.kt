...
class PythonInstallerTest {
    ...
    @InjectMocks
    private lateinit var pythonInstaller: PythonInstaller
    ...
    fun setUp() {
        ...
        val defaultCommand = properties.getProperty("url.default.python")

        ReflectionTestUtils.setField(pythonInstaller, defaultCommandField, defaultCommand)
    }
    ...
    fun `test successful installation with provided package manager`() {
        ...
        verify(shellFormatter).printInfo("Installing Python...")
        verify(strategy).install("python")
    }
    ...
    fun `test default curl URL is valid`() {
        ...
        val modifiedCommand = "curl --head ${pythonInstaller.DEFAULT_URL}"
