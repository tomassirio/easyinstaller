...
class KubectlInstallerTest {
    ...
    @InjectMocks
    private lateinit var kubectlInstaller: KubectlInstaller
    ...
    fun setUp() {
        ...
        val defaultCommand = properties.getProperty("url.default.kubectl")

        ReflectionTestUtils.setField(kubectlInstaller, defaultCommandField, defaultCommand)
    }
    ...
    fun `test successful installation with provided package manager`() {
        ...
        verify(shellFormatter).printInfo("Installing Kubectl...")
        verify(strategy).install("kubectl")
    }
    ...
    fun `test default curl URL is valid`() {
        ...
        val modifiedCommand = "curl --head ${kubectlInstaller.DEFAULT_URL}"
