...
class GitInstallerTest {
    ...
    fun setUp() {
        ...
        val defaultCommand = properties.getProperty("url.default.git")

        ReflectionTestUtils.setField(gitInstaller, defaultCommandField, defaultCommand)
    }
    ...
    fun `test successful installation with default command`() {
        ...
        verify(strategy).invoke(    "mkdir -p /tmp/installer-git && " +
                "cd /tmp/installer-git && " +
                "curl -fsSL ${gitInstaller.DEFAULT_URL} -o git-2.42.0.tar.gz && " +
                "tar -xzvf git-2.42.0.tar.gz && " +
                "cd git-2.42.0 && " +
                "make prefix=/usr/local all && " +
                "sudo make prefix=/usr/local install && " +
                "cd - && " +
                "rm -rf /tmp/installer-git")
    }
    ...
    fun `test default curl URL is valid`() {
        ...
        val modifiedCommand = "curl --head ${gitInstaller.DEFAULT_URL}"
