...
class FishInstallerTest {
    ...
    @InjectMocks
    private lateinit var fishInstaller: FishInstaller
    ...
    fun setUp() {
        ...
        val defaultCommand = properties.getProperty("url.default.fish")

        ReflectionTestUtils.setField(fishInstaller, defaultCommandField, defaultCommand)
    }
    ...
    fun `test successful installation with default command`() {
        ...
        verify(shellFormatter).printInfo("Installing Fish...")
        verify(strategy).invoke("curl -fsSL ${fishInstaller.DEFAULT_URL} | sudo bash")
    }
    ...
    fun `test default curl URL is valid`() {
        ...
        val modifiedCommand = "curl --head ${fishInstaller.DEFAULT_URL}"
