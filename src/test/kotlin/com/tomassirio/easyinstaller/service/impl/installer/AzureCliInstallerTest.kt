...
class AzureCliInstallerTest {
    ...
    @InjectMocks
    private lateinit var azureCliInstaller: AzureCliInstaller
    ...
    fun setUp() {
        ...
        val defaultCommand = properties.getProperty("url.default.azure_cli")

        ReflectionTestUtils.setField(azureCliInstaller, defaultCommandField, defaultCommand)
    }
    ...
    fun `test successful installation with default command`() {
        ...
        azureCliInstaller.install()

        verify(shellFormatter).printInfo("Installing AzureCli...")
        verify(strategy).invoke("curl -fsSL ${azureCliInstaller.DEFAULT_URL} | sudo bash")
    }
    ...
    fun `test successful installation with provided package manager`() {
        ...
        verify(shellFormatter).printInfo("Installing AzureCli...")
        verify(strategy).install("azurecli")
    }
    ...
    fun `test default curl URL is valid`() {
        ...
        val modifiedCommand = "curl --head ${azureCliInstaller.DEFAULT_URL}"
        ...
        assertTrue(output.contains("HTTP/1.1 200 OK")
                .or(output.contains("HTTP/1.1 301")), "Expected output to contain 'HTTP/1.1 200 OK'. Output was: $output")
    }
}