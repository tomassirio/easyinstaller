...
class MkDocsInstallerTest {
    ...
    @InjectMocks
    private lateinit var mkDocsInstaller: MkDocsInstaller
    ...
    fun setUp() {
        ...
        val defaultCommand = properties.getProperty("url.default.mkdocs")

        ReflectionTestUtils.setField(mkDocsInstaller, defaultCommandField, defaultCommand)
    }
    ...
    fun `test successful installation with default command`() {
        ...
        mkDocsInstaller.install()

        verify(shellFormatter).printInfo("Installing MkDocs...")
        verify(strategy).invoke(mkDocsInstaller.DEFAULT_URL)
    }
    ...
    fun `test successful installation with provided package manager`() {
        ...
        verify(shellFormatter).printInfo("Installing MkDocs...")
        verify(strategy).install("mkdocs")
    }
    ...
    fun `test default curl URL is valid`() {
        ...
        val modifiedCommand = "curl --head ${mkDocsInstaller.DEFAULT_URL}"
