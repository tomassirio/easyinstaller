...
class NodeInstallerTest {
    ...
    @InjectMocks
    private lateinit var nodeInstaller: NodeInstaller
    ...
    fun setUp() {
        ...
        val defaultCommand = properties.getProperty("url.default.node")

        ReflectionTestUtils.setField(nodeInstaller, defaultCommandField, defaultCommand)
    }
    ...
    fun `test successful installation with default command`() {
        ...
        verify(shellFormatter).printInfo("Installing Node...")
        verify(strategy).invoke(nodeInstaller.DEFAULT_URL)
    }
    ...
    fun `test default curl URL is valid`() {
        ...
        val modifiedCommand = "curl --head ${nodeInstaller.DEFAULT_URL}"
