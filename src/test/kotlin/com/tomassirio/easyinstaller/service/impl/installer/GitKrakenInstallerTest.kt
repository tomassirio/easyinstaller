...
class GitKrakenInstallerTest {
    ...
    @InjectMocks
    private lateinit var gitKrakenInstaller: GitKrakenInstaller
    ...
    fun setUp() {
        ...
        val defaultCommand = properties.getProperty("url.default.gitkraken")

        ReflectionTestUtils.setField(gitKrakenInstaller, defaultCommandField, defaultCommand)
    }
    ...
    fun `test successful installation with provided package manager`() {
        ...
        verify(shellFormatter).printInfo("Installing GitKraken...")
        verify(strategy).install("gitkraken")
    }
    ...
    fun `test default curl URL is valid`() {
        ...
        val modifiedCommand = "curl --head ${gitKrakenInstaller.DEFAULT_URL}"
