...
class RubyInstallerTest {
    ...
    @InjectMocks
    private lateinit var rubyInstaller: RubyInstaller
    ...
    fun setUp() {
        ...
        val defaultCommand = properties.getProperty("url.default.ruby")

        ReflectionTestUtils.setField(rubyInstaller, defaultCommandField, defaultCommand)
    }
    ...
    fun `test successful installation with default command`() {
        ...
        verify(shellFormatter).printInfo("Installing Ruby...")
        verify(strategy).invoke("curl -fsSL ${rubyInstaller.DEFAULT_URL} | sudo bash -s stable --ruby")
    }
    ...
    fun `test default curl URL is valid`() {
        ...
        val modifiedCommand = "curl --head ${rubyInstaller.DEFAULT_URL}"
