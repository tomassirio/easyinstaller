...
class ScreenInstallerTest {
    ...
    @InjectMocks
    private lateinit var screenInstaller: ScreenInstaller
    ...
    fun setUp() {
        ...
        val defaultCommand = properties.getProperty("url.default.screen")

        ReflectionTestUtils.setField(screenInstaller, defaultCommandField, defaultCommand)
    }
    ...
    fun `test successful installation with default command`() {
        ...
        screenInstaller.install()

        verify(shellFormatter).printInfo("Installing Screen...")
        verify(strategy).invoke(screenInstaller.DEFAULT_URL)
    }
    ...
    fun `test successful installation with provided package manager`() {
        ...
        verify(shellFormatter).printInfo("Installing Screen...")
        verify(strategy).install("screen")
    }
    ...
    fun `test default curl URL is valid`() {
        ...
        val modifiedCommand = "curl --head ${screenInstaller.DEFAULT_URL}"
