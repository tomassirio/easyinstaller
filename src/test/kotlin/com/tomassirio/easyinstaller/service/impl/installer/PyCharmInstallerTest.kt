...
class PyCharmInstallerTest {
    ...
    @InjectMocks
    private lateinit var pyCharmInstaller: PyCharmInstaller
    ...
    fun setUp() {
        ...
        val defaultCommand = properties.getProperty("url.default.pycharm")

        ReflectionTestUtils.setField(pyCharmInstaller, defaultCommandField, defaultCommand)
    }
    ...
    fun `test successful installation with provided package manager`() {
        ...
        verify(shellFormatter).printInfo("Installing PyCharm...")
        verify(strategy).install("pycharm")
    }
    ...
    fun `test default curl URL is valid`() {
        ...
        val modifiedCommand = "curl --head ${pyCharmInstaller.DEFAULT_URL}"
