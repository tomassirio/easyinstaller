...
class RustInstallerTest {
    ...
    @InjectMocks
    private lateinit var rustInstaller: RustInstaller
    ...
    fun setUp() {
        ...
        val defaultCommand = properties.getProperty("url.default.rust")

        ReflectionTestUtils.setField(rustInstaller, defaultCommandField, defaultCommand)
    }
    ...
    fun `test successful installation with default command`() {
        ...
        verify(shellFormatter).printInfo("Installing Rust...")
        verify(strategy).invoke("curl -fsSL ${rustInstaller.DEFAULT_URL} | sudo bash")
    }
    ...
    fun `test default curl URL is valid`() {
        ...
        val modifiedCommand = "curl --head ${rustInstaller.DEFAULT_URL}"
