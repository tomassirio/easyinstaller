...
class GCloudInstallerTest {
    ...
    @InjectMocks
    private lateinit var gCloudInstaller: GCloudInstaller
    ...
    fun setUp() {
        ...
        val defaultCommand = properties.getProperty("url.default.gcloud")

        ReflectionTestUtils.setField(gCloudInstaller, defaultCommandField, defaultCommand)
    }
    ...
    fun `test successful installation with provided package manager`() {
        ...
        verify(shellFormatter).printInfo("Installing GCloud...")
        verify(strategy).install("gcloud")
    }
    ...
    fun `test default curl URL is valid`() {
        ...
        val modifiedCommand = "curl --head ${gCloudInstaller.DEFAULT_URL}"
