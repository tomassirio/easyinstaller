...
class BrewStrategyTest {
    ...
    @InjectMocks
    private lateinit var brewStrategy: BrewStrategy
    ...
    @Test
    fun `install should call process with correct command`() {
        val urlOrName = "example"
        brewStrategy.install(urlOrName)
        verify(shellFormatter).printInfo(commandCaptor.capture())
        assertEquals("With commands: brew install $urlOrName...", commandCaptor.value)
    }

    @Test
    fun `name should return correct name`() {
        assertEquals("brew", brewStrategy.name())
    }
}