...
class DockerInstallerTest {
    ...
    @InjectMocks
    private lateinit var dockerInstaller: DockerInstaller
    ...
    fun setUp() {
        ...
        val defaultCommand = properties.getProperty("url.default.docker")

        ReflectionTestUtils.setField(dockerInstaller, defaultCommandField, defaultCommand)
    }
    ...
    fun `test successful installation with default command`() {
        ...
        verify(shellFormatter).printInfo("Installing Docker...")
        verify(strategy).invoke("curl -fsSL ${dockerInstaller.DEFAULT_URL} | sudo bash")
    }
    ...
    fun `test successful installation with provided package manager`() {
        ...
        verify(shellFormatter).printInfo("Installing Docker...")
        verify(strategy).install("docker")
    }
    ...
    fun `test default curl URL is valid`() {
        ...
        val modifiedCommand = "curl --head ${dockerInstaller.DEFAULT_URL}"
