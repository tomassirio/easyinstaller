...
class BitwardenCliInstallerTest {
    ...
    @InjectMocks
    private lateinit var bitwardenCliInstaller: BitwardenCliInstaller
    ...
    fun setUp() {
        ...
        val defaultCommand = properties.getProperty("url.default.bitwarden_cli")

        ReflectionTestUtils.setField(bitwardenCliInstaller, defaultCommandField, defaultCommand)
    }
    ...
    fun `test successful installation with default command`() {
        ...
        bitwardenCliInstaller.install()

        verify(shellFormatter).printInfo("Installing BitwardenCli...")
        verify(strategy).invoke(bitwardenCliInstaller.DEFAULT_URL)
    }
    ...
    fun `test successful installation with provided package manager`() {
        ...
        verify(shellFormatter).printInfo("Installing BitwardenCli...")
        verify(strategy).install("bitwardencli")
    }
    ...
    fun `test default curl URL is valid`() {
        ...
        val modifiedCommand = "curl --head ${bitwardenCliInstaller.DEFAULT_URL}"
